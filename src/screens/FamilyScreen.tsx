...
import { createKid, deleteKid, Kid, listKids, updateKid } from '../lib/kids';
...
export default function FamilyScreen() {
  ...
  const onSubmit = async () => {
    try {
      ...
      const errorMessage = e instanceof Error ? e.message : 'Unable to save';
      Alert.alert('Error', errorMessage);
    }
  };
  ...
  const remove = async (id: string) => {
    try {
      ...
      const errorMessage = e instanceof Error ? e.message : 'Unable to delete';
      Alert.alert('Error', errorMessage);
    }
  };
  ...
  return (
    <View
      style={{ flex: 1, paddingHorizontal: 20, paddingVertical: 24, gap: 16 }}
    >
      ...
      <Surface elevation={1} style={{ padding: 16, borderRadius: 12, gap: 12 }}>
        ...
        <Button
          mode='contained'
          onPress={onSubmit}
          disabled={kids.length >= 3 && !editingId}
        >
              ...
              >
                ...
                >
                  <IconButton
                    ...
                    size={18}
                    style={{ margin: 0 }}
                    onPress={() => startEdit(kid)}
                  />
                  <IconButton
                    icon='delete'
                    size={18}
                    style={{ margin: 0 }}
                    onPress={() => remove(kid.id)}
                  />
                </View>
              </View>
